...
int initServer(int port) {
	...
	int option = 1;
	setsockopt(sock_fd, SOL_SOCKET, (SO_REUSEPORT | SO_REUSEADDR), &option,
	           sizeof(option));
...
bool checkClient() { return waitingClients.size() > 0; }

int getNumClients() { return clientList.size(); }

int getClientID(int numClient) { return clientList[numClient].id; }
